...
RedactorX.add('block', 'block.tags', {
    ...
    toolbar: {
        ...
        tags: { command: 'tags.edit', title: '## buttons.settings ##', icon: RedactorX.iconTags }
    },
    control: {
...
RedactorX.add('plugin', 'tags', {
    ...
    start: function() {
        this.app.addbar.add('tags', {
            title: '## blocks.tags ##',
            icon: RedactorX.iconTags,
            command: 'tags.popup'
        });
    },
    popup: function() {
        this.app.popup.create('tags', {
            title: '## tags.tags ##',
            width: '100%',
            form: {
        ...
        this.app.popup.open({ focus: 'tags' });
    },
    edit: function(params, button) {
        var popup = this.app.popup.create('tags', {
            title: '## tags.tags ##',
            width: '100%',
            form: {
                tags: { label: '## tags.label ##', type: 'textarea', rows: 4 }
    ...
    save: function(popup) {
        ...
        var instance = this._buildInstance(popup, current);
        if (!instance) {
            current.remove();
        }
    },
    insert: function(popup) {
        var instance = this._buildInstance(popup);
    ...
    // private
    _buildInstance: function(popup, instance) {
