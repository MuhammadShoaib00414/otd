...
class DiscussionReply implements ShouldQueue
{
    ...
    public function __construct(Notification $notification)
    {
        ...
        $this->SMSContent = getSetting('name') . ": ". $notification->notifiable->last_post->owner->name ." replied to a discussion in ". $notification->notifiable->group->name .".\n".config('app.url')."/groups/".$notification->notifiable->group->slug."/discussions/".$notification->notifiable->slug;
    }
    ...
    public function handle()
    {
        ...
        {
            Mail::to($this->notification->user->email)->send(new \App\Mail\DiscussionReply($this->notification->user, $this->notification->notifiable));
            $this->updateNotification();
        }
    }
