...
class EventCancelled implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        elseif($this->notification->user->immediate_notification_method == 'sms' && getsetting('is_event_sms_notifications_enabled'))
        {
            send_sms($this->notification->user->phone, $this->SMSContent);
            $this->updateNotification();
        }
        elseif($this->notification->user->immediate_notification_method == 'email' && EmailNotification::find(6)->is_enabled)
        {
            Mail::to($this->notification->user->email)->send(new \App\Mail\EventCancelled($this->notification->user, $this->notification->notifiable));
            $this->updateNotification();
        }
    }
