...
<div class="container py-3">
  ...
  <div class="row">
    <div class="col-md-12">
      <div class="d-flex align-items-center">
        <h3 class="flex-shrink-0 mb-2">Edit {{ request()->has('isMobile') ? 'Mobile' : 'Desktop'}} Networking Lounge</h3>
        @if(request()->has('isMobile'))
            <span class="ml-3">(<a href="/groups/{{ $group->slug}}/edit-lounge">Switch to desktop</a>)</span>
        @elseif(!$group->lounge->mobile_virtual_room_id)
            <span class="ml-3">(<a href="/groups/{{ $group->slug}}/edit-lounge?isMobile=true">Create mobile version</a>)</span>
        @else
            <span class="ml-3">(<a href="/groups/{{ $group->slug}}/edit-lounge?isMobile=true">Switch to mobile</a>)</span>
        @endif
  ...
  @if(!$room)
    <div class="card">
      <div class="card-body">
          <form method="post" action="/groups/{{ $group->slug}}/edit-lounge/new" enctype="multipart/form-data">
              @csrf
              @method('post')
              @if(request()->has('isMobile'))
    ...
    <div id="roomBuilder">
        <div class="mb-2 d-flex justify-content-between align-items-center">
            ...
            <div>
                ...
                <button @click.prevent="saveAreas()" class="btn btn-outline">Save changes</button>
            </div>
        </div>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form method="post" action="/groups/{{ $group->slug}}/edit-lounge/change-image" enctype="multipart/form-data">
                @csrf
                @method('put')
                @if(request()->has('isMobile'))
                    <input type="hidden" name="is_mobile" value="1">
      ...
      var app = new Vue({
        ...
        methods: {
            ...
            saveAreas() {
                vthis = this;
                $.ajax({
                    url: '/groups/{{ $group->slug}}/edit-lounge/areas',
                    method: 'POST',
                    data: {
                        '_token': '{{ csrf_token() }}',
