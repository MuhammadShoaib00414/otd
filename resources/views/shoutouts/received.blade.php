          ...
          <div class="col-md-9">
            <div class="card">
              <table style="table-layout: fixed;" class="table mb-0">
                <thead>
                  <tr>
                    <td>@lang('shoutouts.shoutout_from')</td>
                ...
                <tbody>
                  @foreach($shoutouts as $shoutout)
                    @if($shoutout->shouted()->exists() && $shoutout->shouting()->exists())
                      <tr>
                        <td>
                          <a href="/users/{{ $shoutout->shouting->id }}" class="d-flex align-items-center justify-content-start">
                            <div class="mr-3" style="height: 3.5em; width: 3.5em; min-height: 3.5em; min-width: 3.5em; border-radius: 50%; background-color: #eee; background-image: url('{{ $shoutout->shouting->photo_path }}'); background-size: cover; background-position: center;">
                            </div>
                            <div>
                              {{ $shoutout->shouting->name }}<br>
                              {{ $shoutout->shouting->job_title }}
                            </div>
                          </a>
                        </td>
                        <td>{{ $shoutout->body }}</td>
                        <td style="font-size: 0.8em;" class="text-right text-muted">{{ $shoutout->created_at->diffForHumans() }}</td>
                      </tr>
                    @endif
                  @endforeach
                </tbody>
