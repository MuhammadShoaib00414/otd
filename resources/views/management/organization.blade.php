...
<div style="background-color: #565d6b;">
  <div class="container-fluid py-3" style="position: relative;">
    <a href="/home" class="d-inline-block mb-2" style="font-size: 14px; position: absolute; top: 50%; color: #fff; transform: translateY(-50%);"><i class="icon-chevron-small-left"></i> @lang('messages.my-dashboard')</a>
        ...
        <div class="card"> 
          <div class="card-body">
            <div class="d-flex justify-content-between">
              <div>
                <a href="/management/my-direct-reports" class="btn btn-outline-secondary">My Direct Reports</a>
                <a href="/management/organization" class="btn btn-secondary">My Organization</a>
              </div>
            ...
            @if(isset($user))
            <h4 class="my-4 font-weight-bold text-center">{{ $user->name }}</h4>
            @else
            <h4 class="my-4 font-weight-bold text-center">My Organization</h4>
            @endif
            <hr>
...
@section('scripts')
  ...
  <script>
    ...
    var introductionsMadeMap = JSON.parse('{!! json_encode($introductionsMade->pluck('id')) !!}');
    ...
    $('#introductionsMade').on('click', function(evt){
      var activePoints = introductionsMade.getElementsAtEvent(evt);
      var selection = activePoints[0]["_index"];

      window.location.href = '/management/organization/' + introductionsMadeMap[selection];
    });
    var introductionsRespondedToMap = JSON.parse('{!! json_encode($introductionsRespondedTo->pluck('id')) !!}');
    ...
    $('#introductionsRespondedTo').on('click', function(evt){
      var activePoints = introductionsRespondedTo.getElementsAtEvent(evt);
      var selection = activePoints[0]["_index"];

      window.location.href = '/management/organization/' + introductionsRespondedToMap[selection];
    });
    var shoutoutsMadeMap = JSON.parse('{!! json_encode($shoutoutsMade->pluck('id')) !!}');
    ...
    $('#shoutoutsMade').on('click', function(evt){
      var activePoints = shoutoutsMade.getElementsAtEvent(evt);
      var selection = activePoints[0]["_index"];

      window.location.href = '/management/organization/' + shoutoutsMadeMap[selection];
    });
    var shoutoutsReceivedMap = JSON.parse('{!! json_encode($shoutoutsReceived->pluck('id')) !!}');
    ...
    $('#shoutoutsReceived').on('click', function(evt){
      var activePoints = shoutoutsReceived.getElementsAtEvent(evt);
      var selection = activePoints[0]["_index"];

      window.location.href = '/management/organization/' + shoutoutsReceivedMap[selection];
    });
    ...
    var skillsetsOfferedPerDirectReportMap = JSON.parse('{!! json_encode($skillsetsPerPerson->pluck('id')) !!}');
    ...
    $('#skillsetsOfferedPerDirectReport').on('click', function(evt){
      var activePoints = skillsetsOfferedPerDirectReport.getElementsAtEvent(evt);
      var selection = activePoints[0]["_index"];

      window.location.href = '/management/organization/' + skillsetsOfferedPerDirectReportMap[selection];
    });
    ...
    var seekingMentorshipMap = JSON.parse('{!! json_encode($seekingMentorship->pluck('id')) !!}');
    ...
    $('#seekingMentorship').on('click', function(evt){
      var activePoints = seekingMentorship.getElementsAtEvent(evt);
      var selection = activePoints[0]["_index"];

      window.location.href = '/management/organization/' + seekingMentorshipMap[selection];
    });
