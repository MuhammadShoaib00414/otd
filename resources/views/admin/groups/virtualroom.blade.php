...
@section('inner-page-content')
  @if(!$room)
      <div class="card">
        <div class="card-body">
            <form method="post" action="/admin/groups/{{ $group->id }}/virtual-room/new" enctype="multipart/form-data">
                @csrf
                @method('post')
                ...
                <div class="form-group">
                    <label for="photoUpload">Upload header image</label>
  ...
  <div class="d-flex mb-2">
    <h5>Edit {{ request()->has('isMobile') && request()->isMobile == true ? 'Mobile' : 'Desktop'}} Interactive Header Image</h5>
    <span> (
            @if(request()->has('isMobile') && request()->isMobile == true)
                <a href="/admin/groups/{{ $group->id }}/virtual-room">Switch to desktop version</a>
            @else
                <a href="/admin/groups/{{ $group->id }}/virtual-room?isMobile=true">{{ $group->mobile_virtual_room ? 'Switch to mobile version' : 'Create mobile version'}}</a>
            @endif
            )
    </span>
  </div>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form method="post" action="/admin/groups/{{ $group->id }}/virtual-room/change-image" enctype="multipart/form-data">
                @csrf
                @method('put')
      ...
      var app = new Vue({
        ...
        methods: {
            ...
            saveAreas() {
                vthis = this;
                $.ajax({
                    url: '/admin/groups/{{ $group->id }}/virtual-room/areas',
                    method: 'POST',
                    data: {
                        @if(request()->has('isMobile'))
                        'is_mobile': 'true',
