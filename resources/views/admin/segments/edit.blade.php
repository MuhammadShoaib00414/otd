...
@section('page-content')
    ...
    <div class="row mb-5" id="filterBuilder">
        <div class="col-md-6">
            <form action="/admin/segments/{{ $segment->id }}" method="post">
                @csrf
                @method('put')
                <div class="form-group">
                    <label for="name">Name</label>
                    <input type="text" id="name" name="name" class="form-control" value="{{ $segment->name }}" maxlength="254" required>
                </div>
                <div class="form-group">
                    <label for="name">Date Range</label>
                    <input type="text" name="daterange" value="{{ ($segment->start_date) ? $segment->start_date->format('m/d/Y') : Carbon\Carbon::now()->subDays(30)->format('m/d/Y')  }} - {{ ($segment->start_date) ? $segment->end_date->format('m/d/Y') : Carbon\Carbon::now()->format('m/d/Y') }}" class="form-control" style="width: 400px;" />
                ...
                <div class="form-group">
                    ...
                    @foreach($groups as $group)
                        @include('partials.groupcheckbox', [
                            'group' => $group,
                            'checked' => ($segment && isset($segment->filters) && isset($segment->filters->groups) && in_array($group->id, $segment->filters->groups)),
                            'segment' => $segment,
                        ])
                    @endforeach
                </div>
                ...
                <div class="filters">
                    <div v-for="(filter, index) in filterObject">
                        <div class="form-row mb-3 filter-row">
                            ...
                            <div class="col-md-4">
                                <div class="d-flex">
                                    <select class="custom-select" :name="'filters['+index+'][parameter]'" v-model="filter.value" v-if="doesTypeHaveOptions(filter.type)">
